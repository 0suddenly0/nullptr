...
/*
 ...
 - 2017/10/11 (1.52) - renamed AlignFirstTextHeightToWidgets() to AlignTextToFramePadding(). Kept redirection function (will obsolete).
 ...
 - 2017/08/22 (1.51) - renamed IsItemHoveredRect() to IsItemRectHovered(). Kept redirection function (will obsolete). -> (1.52) use IsItemHovered(ImGuiHoveredFlags_RectOnly)!
                     - renamed IsMouseHoveringAnyWindow() to IsAnyWindowHovered() for consistency. Kept redirection function (will obsolete).
                     - renamed IsMouseHoveringWindow() to IsWindowRectHovered() for consistency. Kept redirection function (will obsolete).
 ...
 - 2016/03/21 (1.48) - renamed GetWindowFont() to GetFont(), GetWindowFontSize() to GetFontSize(). Kept redirection function (will obsolete).
 ...
 - 2015/07/02 (1.42) - renamed SetScrollPosHere() to SetScrollFromCursorPos(). Kept redirection function (will obsolete).
 - 2015/07/02 (1.42) - renamed GetScrollPosY() to GetScrollY(). Necessary to reduce confusion along with other scrolling functions, because positions (e.g. cursor position) are not equivalent to scrolling amount.
 - 2015/06/14 (1.41) - changed ImageButton() default bg_col parameter from (0,0,0,1) (black) to (0,0,0,0) (transparent) - makes a difference when texture have transparence
 - 2015/06/14 (1.41) - changed Selectable() API from (label, selected, size) to (label, selected, flags, size). Size override should have been rarely be used. Sorry!
 - 2015/05/31 (1.40) - renamed GetWindowCollapsed() to IsWindowCollapsed() for consistency. Kept redirection function (will obsolete).
 - 2015/05/31 (1.40) - renamed IsRectClipped() to IsRectVisible() for consistency. Note that return value is opposite! Kept redirection function (will obsolete).
 - 2015/05/27 (1.40) - removed the third 'repeat_if_held' parameter from Button() - sorry! it was rarely used and inconsistent. Use PushButtonRepeat(true) / PopButtonRepeat() to enable repeat on desired buttons.
 - 2015/05/11 (1.40) - changed BeginPopup() API, takes a string identifier instead of a bool. ImGui needs to manage the open/closed state of popups. Call OpenPopup() to actually set the "open" state of a popup. BeginPopup() returns true if the popup is opened.
 - 2015/05/03 (1.40) - removed style.AutoFitPadding, using style.WindowPadding makes more sense (the default values were already the same).
 - 2015/04/13 (1.38) - renamed IsClipped() to IsRectClipped(). Kept redirection function until 1.50.
 - 2015/04/09 (1.38) - renamed ImDrawList::AddArc() to ImDrawList::AddArcFast() for compatibility with future API
 - 2015/04/03 (1.38) - removed ImGuiCol_CheckHovered, ImGuiCol_CheckActive, replaced with the more general ImGuiCol_FrameBgHovered, ImGuiCol_FrameBgActive.
 - 2014/04/03 (1.38) - removed support for passing -FLT_MAX..+FLT_MAX as the range for a SliderFloat(). Use DragFloat() or Inputfloat() instead.
 - 2015/03/17 (1.36) - renamed GetItemBoxMin()/GetItemBoxMax()/IsMouseHoveringBox() to GetItemRectMin()/GetItemRectMax()/IsMouseHoveringRect(). Kept redirection function until 1.50.
 - 2015/03/15 (1.36) - renamed style.TreeNodeSpacing to style.IndentSpacing, ImGuiStyleVar_TreeNodeSpacing to ImGuiStyleVar_IndentSpacing
 - 2015/03/13 (1.36) - renamed GetWindowIsFocused() to IsWindowFocused(). Kept redirection function until 1.50.
 - 2015/03/08 (1.35) - renamed style.ScrollBarWidth to style.ScrollbarWidth (casing)
 - 2015/02/27 (1.34) - renamed OpenNextNode(bool) to SetNextTreeNodeOpened(bool, ImGuiSetCond). Kept redirection function until 1.50.
...
static bool             BeginChildExE(const char* name, ImGuiID id, const ImVec2& size_arg, const ImVec2& pos_arg, bool border, ImGuiWindowFlags flags);
...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
	...
	ImVec4 becup_color = g.Style.Colors[ImGuiCol_Border];

	if (flags & ImGuiWindowFlags_ChildWindowTitle)
	{
		//g.Style.Colors[ImGuiCol_Border] = ImVec4(1.00f, 1.00f, 1.00f, 0.00f);

		//parent_window->DrawList->AddRectFilled(ImVec2(child_window->Pos.x, child_window->Pos.y + (name_size.y / 2) - 1), ImVec2(child_window->Pos.x + 15, child_window->Pos.y + (name_size.y / 2) + 1), GetColorU32(ImVec4(1.f, 0.2f, 0.2f, 1.f)));
		//parent_window->DrawList->AddRectFilled(ImVec2(child_window->Pos.x, child_window->Pos.y + (name_size.y / 2) - 1), ImVec2(child_window->Pos.x + child_window->Size.x, child_window->Pos.y + (name_size.y / 2) + 1), GetColorU32(ImVec4(1.f, 0.2f, 0.2f, 1.f)));
	}
	...
	g.Style.Colors[ImGuiCol_Border] = becup_color;
...
static bool ImGui::BeginChildExE(const char* name, ImGuiID id, const ImVec2& size_arg, const ImVec2& pos_arg, bool border, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* parent_window = g.CurrentWindow;
    ...
    ImVec4 becup_color = g.Style.Colors[ImGuiCol_Border];
    ImVec4 becup_color_c = g.Style.Colors[ImGuiCol_ChildBg];
    ...
    g.Style.ChildBorderSize = backup_border_size;
    g.Style.Colors[ImGuiCol_Border] = becup_color;
    g.Style.Colors[ImGuiCol_ChildBg] = becup_color_c;
    ...
    SetWindowPos(child_window->Pos + pos_arg);

    parent_window->DrawList->AddRectFilled(child_window->Pos, child_window->Pos + child_window->Size, GetColorU32(ImGuiCol_ChildBg));
...
bool ImGui::BeginChildS(const char* str_id, const ImVec2& size_arg, const ImVec2& pos_arg, bool border, ImGuiWindowFlags extra_flags)
{
    /*ImGuiWindow* window = GetCurrentWindow();
    return BeginChildEx(str_id, window->GetID(str_id), size_arg, border, extra_flags);*/
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ImGui::Dummy(ImVec2(0, 1));
    bool ret = BeginChildExE(str_id, window->GetID(str_id), size_arg, pos_arg, border, extra_flags);
...
void ImGui::EndChild(const char* name)
{
    ...
    {
        //window->DrawList->AddRect(window->Pos, window->Pos + window->Size, ImGui::GetColorU32(ImVec4(1.f, 1.f, 1.f, 1.f)),0.f, 15, 1.f);
        //window->DrawList->AddRect(window->Pos, window->Pos + ImVec2(0, window->Size.y), ImGui::GetColorU32(ImVec4(1.f, 1.f, 1.f, 1.f)));
        //window->DrawList->AddRect(window->Pos + ImVec2(0, window->Size.y + 1), window->Pos + window->Size + ImVec2(0, 1), ImGui::GetColorU32(ImVec4(1.f, 1.f, 1.f, 1.f)));
    ...
    /*if (window->Flags & ImGuiWindowFlags_ListBoxWindowTitle)
    {
        float offset_y = name_size.y + 2;
        ImVec2 offset = ImVec2(0, offset_y);
        ImVec4 Color_list = ImVec4(0.4f, 0.4f, 0.4f, 1.f);
        window->DrawList->AddRectFilled(window->Pos - offset, window->Pos + ImVec2(window->Size.x, 0), ImGui::GetColorU32(ImGuiCol_TitleBg));
        window->DrawList->AddText(ImVec2(window->Pos.x, window->Pos.y - offset_y), GetColorU32(ImGuiCol_Text), window->Name, FindRenderedTextEnd(window->Name, NULL));
    }*/
}
...
/*ImGuiComboBox* ImGui::CreateCombo(const char* name, ImVec2 size)
{
    ImGuiContext& g = *GImGui;
...
static ImVec2 CalcSizeAfterConstraint(ImGuiWindow* window, ImVec2 new_size)
{
    ...
    if (g.NextWindowData.SizeConstraintCond != 0)
    {
        ...
        ImRect cr = g.NextWindowData.SizeConstraint_rect;
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if (!window_pos_set_by_api && !(flags & ImGuiWindowFlags_ChildWindow) && window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0)
        {
            ...
            {
                ImVec2 padding = window->Size;//ImMax(style.DisplayWindowPadding, style.DisplaySafeAreaPadding);
                ImVec2 size_for_clamping = window->Size;//((g.IO.ConfigWindowsMoveFromTitleBarOnly) && !(window->Flags & ImGuiWindowFlags_NoTitleBar)) ? ImVec2(window->Size.x, window->TitleBarHeight()) : window->Size;
        ...
        window->WindowRounding = (flags & ImGuiWindowFlags_ListBoxWindowTitle) ? style.FrameRounding : (flags & ImGuiWindowFlags_ChildWindow) ? style.ChildRounding : ((flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiWindowFlags_Modal)) ? style.PopupRounding : style.WindowRounding;
        ...
        else
        {
            ...
            if (!(flags & ImGuiWindowFlags_NoBackground))
            {
                ...
                window->DrawList->AddRectFilled(window->Pos + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size, bg_col, window_rounding, (flags & ImGuiWindowFlags_ListBoxWindowTitle) ? ImDrawCornerFlags_Bot : (flags & ImGuiWindowFlags_NoTitleBar) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Bot);
            }
            ...
            if (flags & ImGuiWindowFlags_RainbowTitleBar && !(flags & ImGuiWindowFlags_NoTitleBar))
                window->DrawList->AddRectFilled(title_bar_rect.Min, ImVec2(title_bar_rect.Max.x, title_bar_rect.Min.y + 4.f), GetColorU32(ImGuiCol_SliderGrab), window_rounding, ImDrawCornerFlags_Top);
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar))
        {
            ...
            if (p_open != NULL)
            {
                const float pad = style.FramePadding.y + 2.f;
            ...
            RenderTextClipped(text_r.Min + ImVec2(0, 4.f), text_r.Max, name, NULL, &text_size, style.WindowTitleAlign, &clip_rect);
        ...
        */
        ...
        /*ImVec2 name_size = CalcTextSize(name);
        float new_border_top = name_size.y / 2;

        float top_border_size = (((flags & ImGuiWindowFlags_MenuBar) || !(flags & ImGuiWindowFlags_NoTitleBar)) ? style.FrameBorderSize : (flags & ImGuiWindowFlags_ChildWindowTitle) ? new_border_top : -style.WindowBorderSize);
        window->InnerClipRect.Min.x = ImFloor(0.5f + window->InnerMainRect.Min.x + ImMax(ImFloor(window->WindowPadding.x * 0.5f), window->WindowBorderSize));
        window->InnerClipRect.Min.y = ImFloor(0.5f + window->InnerMainRect.Min.y);
        window->InnerClipRect.Max.x = ImFloor(0.5f + window->InnerMainRect.Max.x - ImMax(ImFloor(window->WindowPadding.x * 0.5f), window->WindowBorderSize));
        window->InnerClipRect.Max.y = ImFloor(0.5f + window->InnerMainRect.Max.y - window->WindowBorderSize);*/
        ...
        // Inner clipping rectangle
        // Force round operator last to ensure that e.g. (int)(max.x-min.x) in user's render code produce correct result.
        if (!(flags & ImGuiWindowFlags_RainbowTitleBar))
        {
            window->InnerClipRect.Min.x = ImFloor(0.5f + window->InnerMainRect.Min.x + ImMax(0.0f, ImFloor(window->WindowPadding.x * 0.5f - window->WindowBorderSize)));
            window->InnerClipRect.Min.y = ImFloor(0.5f + window->InnerMainRect.Min.y);
            window->InnerClipRect.Max.x = ImFloor(0.5f + window->InnerMainRect.Max.x - ImMax(0.0f, ImFloor(window->WindowPadding.x * 0.5f - window->WindowBorderSize)));
        ...
        {
            window->InnerClipRect.Min.x = ImFloor(window->InnerMainRect.Min.x);
            window->InnerClipRect.Min.y = ImFloor(window->InnerMainRect.Min.y);
            window->InnerClipRect.Max.x = ImFloor(window->InnerMainRect.Max.x);
            window->InnerClipRect.Max.y = ImFloor(window->InnerMainRect.Max.y);
        }

        if (flags & ImGuiWindowFlags_ChildWindowTitle)
        {
            float new_border_top = 17;

            float top_border_size = new_border_top;
            window->InnerClipRect.Min.x = ImFloor(0.5f + window->InnerMainRect.Min.x + ImMax(ImFloor(window->WindowPadding.x * 0.5f), window->WindowBorderSize));
            window->InnerClipRect.Min.y = ImFloor(0.5f + window->InnerMainRect.Min.y + top_border_size);
            window->InnerClipRect.Max.x = ImFloor(0.5f + window->InnerMainRect.Max.x - ImMax(ImFloor(window->WindowPadding.x * 0.5f), window->WindowBorderSize));
            window->InnerClipRect.Max.y = ImFloor(0.5f + window->InnerMainRect.Max.y - window->WindowBorderSize);
        }
...
static const ImGuiStyleVarInfo GStyleVarInfo[] =
{
    ...
    { ImGuiDataType_Float, 2, (ImU32)IM_OFFSETOF(ImGuiStyle, ItemInnerSpacing_new) },// ImGuiStyleVar_ItemInnerSpacing
...
void ImGui::SetNextWindowSizeConstraints(const ImVec2& size_min, const ImVec2& size_max, ImGuiSizeCallback custom_callback, void* custom_callback_user_data)
{
    ...
    g.NextWindowData.SizeConstraint_rect = ImRect(size_min, size_max);
...
{
    ImGuiContext& g = *GImGui;
    return g.FontSize + g.Style.FramePadding.y * 2.0f - 2.f;
}
...
void ImGui::SameLine(float pos_x, float spacing_w)
{
    ...
    {
        if (spacing_w < 0.0f) spacing_w = g.Style.ItemInnerSpacing_new.x;
...
bool ImGui::BeginPopup(const char* str_id, ImGuiWindowFlags flags)
{
    ...
    flags |= ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSpaceAnyItems;
...
static float GetColumnsRectHalfWidth() { return 4.0f; }
