...
namespace render
{
	...
	float draw_text(const std::string& text, ImVec2 position, float size, ImVec4 color, bool center = false, bool outline = true, ImFont* pFont = default_font);
	...
	template <class T>
	void draw_box_filled_by_type(T x1, T y1, T x2, T y2, Color color, float thickness = 1.f, int type = 0) {
		if (type == 0 || type == 1)
			draw_box_filled(x1, y1, x2, y2, color, thickness);
		else if (type == 2)
			draw_box_filled(x1, y1, x2, y2, color, thickness, 8.f);
	}
	...
	template <class T>
	void draw_box_filled(T x1, T y1, T x2, T y2, Color color, float thickness = 1.f, float rounding = 0.f) {
		draw_list->AddRectFilled(ImVec2(x1, y1), ImVec2(x2, y2), get_u32(color), rounding, 15);
	}
	template <class T>
	void draw_box_filled_round(T x1, T y1, T x2, T y2, Color color, float side = 1.f, float rounding = 0.f)
	{
		draw_list->AddRectFilled(ImVec2(x1, y1), ImVec2(x2, y2), get_u32(color), rounding, side == 1 ? ImDrawCornerFlags_Left : ImDrawCornerFlags_Right);
	}
	template <class T>
	void draw_box_filled_multicolor(T x1, T y1, T x2, T y2, Color color_left, Color color_right) {
	...
	template <class T>
	void draw_box_filled_multicolor(T x1, T y1, T x2, T y2, Color color_left_top, Color color_left_bottom, Color color_right_top, Color color_right_bottom) {
	...
	template <class T>
	void draw_box_filled(T x1, T y1, T x2, T y2, ImVec4 color, float thickness = 1.f, float rounding = 0.f) {
	...
	template <class T>
	float draw_text(const std::string& text, T x, T y, float size, ImVec4 clr, bool center = false, bool outline = true, ImFont* pFont = default_font) {
		return draw_text(text, ImVec2(x, y), size, clr, center, outline, pFont);
	}
	template <class T>
