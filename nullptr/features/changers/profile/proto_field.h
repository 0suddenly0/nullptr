...
#define MAKE_TAG(FIELD_NUMBER, TYPE) static_cast<uint32_t>(((FIELD_NUMBER) << c_field::TagTypeBits) | (TYPE))

enum FieldType_iaeblan {
	_TYPE_DOUBLE = 1,
	_TYPE_FLOAT = 2,
	_TYPE_INT64 = 3,
	_TYPE_UINT64 = 4,
...
struct c_field
{
	...
	const wire_type_t kWireTypeForFieldType[_MAX_FIELD_TYPE + 1] = {
		static_cast<wire_type_t>(-1),  // invalid
	...
	std::string full;
public:
	c_field& operator=(const c_field& f) {
		this->tag = f.tag;
		this->value = f.value;
		this->full = f.full;
		return *this;
	}
	c_field() { tag = { 0,0 }; value = ""; full = ""; }
	c_field(unsigned field, unsigned type, std::string value, std::string full) {
		this->tag = { field, type };
		this->value = value;
		this->full = full;
	}
	c_field(unsigned field, unsigned type, float value) {
	...
	c_field(unsigned field, unsigned type, double value) {
		auto wireType = kWireTypeForFieldType[type];
		tag = { field, (unsigned)wireType };
		full = getBytesVarint32(MAKE_TAG(field, wireType));
		full += std::string{ reinterpret_cast<const char*>(&value), 8 };
	}
	c_field(unsigned field, unsigned type, int64_t value) {
		auto wireType = kWireTypeForFieldType[type];
		tag = { field, (unsigned)wireType };
		full = getBytesVarint32(MAKE_TAG(field, wireType));
		switch (wireType) {
	...
	c_field(unsigned field, unsigned type, std::string value) {
		auto wireType = kWireTypeForFieldType[type];
		tag = { field, (unsigned)wireType };
		full = getBytesVarint32(MAKE_TAG(field, wireType));
...
public:
	...
	static c_field read(void* data, size_t & bytesRead)
	{
		...
		return c_field(field, type, value, full);
	}
