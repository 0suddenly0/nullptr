...
namespace engine_prediction
{
	namespace type2
	{
		...
		{
			static auto oldorigin = g_local_player->vec_origin();
			oldorigin = g_local_player->vec_origin();
			...
			if (pLastCmd)
			{
				if (pLastCmd->hasbeenpredicted)
					flTickBase = g_local_player->tick_base();
				else
					++flTickBase;
			}
			...
			static auto pred_oldorigin = g_local_player->vec_origin();
			pred_oldorigin = g_local_player->vec_origin();
		}
	...
	namespace type1
	{
		...
		void begin(c_usercmd* cmd)
		{
			...
			if (!_prevcmd || _prevcmd->hasbeenpredicted) {
				_fixedtick = g_local_player->tick_base();
			}
			else {
				_fixedtick++;
			}
			...
			g_local_player->current_command() = cmd;
			...
			*(bool*)((uintptr_t)g_prediction + 0x8) = _inpred_backup;
			...
			g_local_player->current_command() = nullptr;
