...
namespace math
{
	...
	void movement_fix( c_usercmd* m_Cmd, QAngle wish_angle, QAngle old_angles ) {
		if( old_angles.pitch != wish_angle.pitch || old_angles.yaw != wish_angle.yaw || old_angles.roll != wish_angle.roll ) {
			...
			if( !( g_local_player->m_flags( ) & FL_ONGROUND ) && viewangles.roll != 0.f )
				movedata.y = 0.f;
	...
	bool world_to_screen_ofc(const Vector& in, Vector& out)
	{
		if (!math::world_to_screen(in, out))
			return false;

		static int iScreenWidth, iScreenHeight;
		g_engine_client->get_screen_size(iScreenWidth, iScreenHeight);

		auto xOk = iScreenWidth > out.x > 0.0f;
		auto yOk = iScreenHeight > out.y > 0.0f;
