...
sound::WavHeader hitsoundHeader;

namespace sound
{
	void load_wav(std::string fname, BYTE** out)
	{
	...
	void WavHeader::parseWavHeader(unsigned char* wavData)
	{
	...
	bool adjustVolume(WavHeader& header, float volume, float volRatio)
	{
		...
		float finalVolume = std::clamp(volume * volRatio, 0.f, 1.f);
	...
	void change_volume()
	{
		//memcpy(hitsound, soundSrc, srcLength);

		static bool inalizeaseeas = false;

		if (!inalizeaseeas)
		{
			load_wav("C:\\nullptr\\skeet.wav", &globals::hit_sound_byte);
			hitsoundHeader.parseWavHeader(globals::hit_sound_byte);
			inalizeaseeas = true;
		}
	

		old_volume = settings::misc::hitsound::volume;

		// My hitsound is actually kind of quiet compared to CS 
	   // so I have used a value of 2.3 for volumeRatio
		adjustVolume(hitsoundHeader, old_volume, 1.f);
